...
export const SplashCursor: React.FC<SplashCursorProps> = ({
  ...
  TRANSPARENT = true,
  RAINBOW_MODE = true,
  COLOR = '#5d46d8' // Use Unexpected Solutions brand purple as default
}) => {
  ...
  useEffect(() => {
    ...
    const pointers = [new PointerInstance()];
    ...
    function getWebGLContext(canvasElement: HTMLCanvasElement) {
      ...
      let glCtx = canvasElement.getContext('webgl2', params) as WebGL2RenderingContext | null;
      const isWebGL2 = !!glCtx;
      if (!isWebGL2) {
        glCtx = (canvasElement.getContext('webgl', params) || canvasElement.getContext('experimental-webgl', params)) as any;
      }
    ...
    function getSupportedFormat(glCtx: any, internalFormat: number, format: number, type: number, supportLinearFiltering: any) {
    ...
    function getUniforms(prog: WebGLProgram) {
      ...
      for (let i = 0; i < uniformCount; i++) {
        const activeUniform = gl.getActiveUniform(prog, i);
        if (activeUniform) {
          const uniformName = activeUniform.name;
          const location = gl.getUniformLocation(prog, uniformName);
    ...
    function updatePointerMoveData(pointer: any, posX: number, posY: number, color: ColorRGB) {
      pointer.prevTexcoordX = pointer.texcoordX;
      pointer.prevTexcoordY = pointer.texcoordY;
      pointer.texcoordX = posX / canvas.width;
      pointer.texcoordY = 1.0 - posY / canvas.height;
  ...
  return (
    <div
      style={{
        ...
        top: 0,
        left: 0,
        zIndex: 9999, // Render on top of everything to handle screen cursor trail
        pointerEvents: 'none',
        width: '100%',
        height: '100%'
      }}
